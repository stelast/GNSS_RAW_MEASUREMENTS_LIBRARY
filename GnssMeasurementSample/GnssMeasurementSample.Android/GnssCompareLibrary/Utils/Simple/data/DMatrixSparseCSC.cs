...
namespace GnssMeasurementSample.Droid.GnssCompareLibrary.Utils.Simple.data
{
    ...
    public class DMatrixSparseCSC : DMatrixSparse
    {
         ...
         */
        public DMatrixSparseCSC(int numRows, int numCols) : this(numRows, numCols, 0)
        {
        ...
        public DMatrixSparseCSC(int numRows, int numCols, int arrayLength)
        {
            if (numRows < 0 || numCols < 0 || arrayLength < 0)
                throw new ArgumentException("Rows, columns, and arrayLength must be not be negative");
        ...
        public virtual DMatrixSparseCSC copy()
        {
            return new DMatrixSparseCSC(this);
        }
        
        public virtual DMatrixSparseCSC createLike()
        {
        ...
        public virtual double get(int row, int col, double fallBackValue)
        {
            if (row < 0 || row >= numRows || col < 0 || col >= numCols)
                throw new ArgumentException("Outside of matrix bounds");
        ...
        public int nz_index(int row, int col)
        {
            ...
            {
                return Array.BinarySearch(nz_rows, col0, col1, row);
            }
            else
            {
        ...
        {
            Array.Fill(col_idx, 0, numCols + 1, 0);
            nz_length = 0;
            indicesSorted = false; // see justification in reshape
        }
        
        public virtual DMatrixSparseCSC create(int numRows, int numCols)
        {
            return new DMatrixSparseCSC(numRows, numCols);
        }
        
        public virtual int getNonZeroLength()
        {
            return nz_length;
        }
        ...
        public virtual void shrinkArrays()
        {
            if (nz_length < nz_values.Count())
            {
                ...
                System.Array.Copy(this.nz_values, 0, tmp_values, 0, nz_length);
                System.Array.Copy(this.nz_rows, 0, tmp_rows, 0, nz_length);
        ...
        {
            if (arrayLength < 0)
                throw new ArgumentException("Negative array length. Overflow?");
            ...
            //        }
            if (arrayLength > this.nz_values.Count())
            {
                ...
                if (preserveValue)
                {
                    System.Array.Copy(this.nz_values, 0, data, 0, this.nz_length);
                    System.Array.Copy(this.nz_rows, 0, row_idx, 0, this.nz_length);
                }
        ...
        {
            if (col_idx.Count() < desiredColumns + 1)
            {
                int[] c = new int[desiredColumns + 1];
                if (preserveValue)
                    System.Array.Copy(col_idx, 0, c, 0, col_idx.Count());
                col_idx = c;
            }
        }
        ...
        public void histogramToStructure(int[] histogram)
        {
            ...
            if (col_idx[numCols] != nz_length)
                throw new SystemException("Egads");
        }
        ...
        public int NonZeroCount => throw new NotImplementedException();
        ...
        IEnumerator<CoordinateRealValue> DMatrixSparse.createCoordinateIterator()
        {
            throw new NotImplementedException();
        }
        
        public virtual int getNonZeroCount()
        {
            return nz_length;
        }
