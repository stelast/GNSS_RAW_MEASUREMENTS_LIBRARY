...
namespace GnssMeasurementSample.Droid.GnssCompareLibrary.Utils.Simple.ops
{
    public class SymmetricQREigenHelper_DDRM
    {
        ...
        {
            if (ret == null || ret.Count() < N)
            {
                ret = new double[N];
            }

            System.Array.Copy(diag, 0, ret, 0, N);
        ...
        {
            if (ret == null || ret.Count() < N - 1)
            {
                ret = new double[N - 1];
            }

            System.Array.Copy(off, 0, ret, 0, N - 1);
         ...
         */
        protected bool isZero(int index)
        {
        ...
        protected void performImplicitSingleStep(double lambda, bool byAngle)
        {
            if (x2 - x1 == 1)
            {
