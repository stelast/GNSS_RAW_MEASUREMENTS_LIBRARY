...
namespace GnssMeasurementSample.Droid.GnssCompareLibrary.Utils.Simple.ops
{
    public class DConvertMatrixStruct
    {
        ...
        public static DMatrix2 convert(DMatrixRMaj input,  DMatrix2 output )
        {
            ...
            if (input.NumRows != 1 && input.NumCols != 1)
                throw new ArgumentException("One row or column must have a length of 1 for it to be a vector");
            int length = Math.Max(input.NumRows, input.NumCols);
            if (length != 2)
                throw new ArgumentException("Length of input vector is not 2.  It is " + length);
        ...
        public static void convert(DMatrixRMaj src, DMatrixRBlock dst)
        {
            ...
            for (int i = 0; i < dst.numRows; i += dst.blockLength)
            {
                ...
                for (int j = 0; j < dst.numCols; j += dst.blockLength)
                {
                    ...
                    {
                        System.Array.Copy(src.data, indexSrcRow, dst.data, indexDst, blockWidth);
                        indexDst += blockWidth;
        ...
        {
            if (dst == null)
                dst = new DMatrixSparseTriplet(src.NumRows, src.NumCols, 1);
            else
                dst.reshape(src.NumRows, src.NumCols);
